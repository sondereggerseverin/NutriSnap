...
data class UserProfileEntity(
    ...
    val weightKg: Float = 0f, val heightCm: Int = 0, val ageYears: Int = 0,
    val dailyCalorieGoal: Int = 2000, val proteinGoalG: Float = 120f,
    val carbsGoalG: Float = 220f, val fatGoalG: Float = 65f, val activityFactor: Float = 1.55f
)

fun UserProfileEntity.toDomain() = UserProfile(weightKg, heightCm, ageYears, dailyCalorieGoal, proteinGoalG, carbsGoalG, fatGoalG, activityFactor)
fun UserProfile.toEntity() = UserProfileEntity(weightKg = weightKg, heightCm = heightCm, ageYears = ageYears, dailyCalorieGoal = dailyCalorieGoal, proteinGoalG = proteinGoalG, carbsGoalG = carbsGoalG, fatGoalG = fatGoalG, activityFactor = activityFactor)
...
@Database(
    entities = [
        FoodItem::class, DiaryEntry::class, Recipe::class, UserProfileEntity::class,
        WeightEntry::class, FavoriteFoodEntity::class,
        WaterEntry::class, FastingSession::class, RecipeCollection::class
    ],
    version = 5,
    exportSchema = false
)
abstract class NutriDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_4_5 = object : Migration(4, 5) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // Rezept-Sammlungen
                db.execSQL("CREATE TABLE IF NOT EXISTS recipe_collections (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name TEXT NOT NULL, emoji TEXT NOT NULL DEFAULT '📁', createdAt INTEGER NOT NULL)")
        ...
        fun getInstance(context: Context): NutriDatabase =
            INSTANCE ?: synchronized(this) {
                Room.databaseBuilder(context.applicationContext, NutriDatabase::class.java, "nutrisnap.db")
